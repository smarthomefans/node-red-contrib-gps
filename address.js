module.exports = function (RED) {
    ...
    function address(config) {
        ...
        if (this.server) {
            ...
            node.status({fill:"red", shape:"ring", text: "没有配置正确的汽车在线信息"});
            return
        }
    ...
    function getDataFromHttp(node , server, msg, config){
        ...
        axios({
            method: 'get',
            url: `http://api.gpsoo.net/1/tool/address?lng=${longitude}&lat=${latitude}&access_token=${access_token}`,
            headers: {
    ...
    RED.nodes.registerType("gpsoo-address", address);
